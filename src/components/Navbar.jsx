...
const Navbar = () => {
  ...
  return (
    <div className='fixed z-50 w-full px-6 md:px-20 py-3 md:py-[25px] font-["Neue_Montreal"] flex justify-between items-center'>
      <div className='logo ml-8'>
      <svg className='mt-2 md:inline hidden' width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="md:hidden absolute left-0 top-0 mt-6 ml-3">
        <button
      ...
      <nav className="hidden md:flex gap-10 ">
        {["Services", "Our Works", "About Us", "Insights", "Contact"].map(
          (item, index) => (
            <a
              key={index}
              className={`text text-lg capitalize font-light text-white ${index === 4 ? "ml-32" : ""}`}
            >
              {item}
            </a>
          )
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed top-[80px] left-0 right-0 shadow-lg">
            ...
            {["Services", "Our Works", "About Us", "Insights", "Contact"].map(
              (item, index) => (
                <a
                  key={index}
                  onClick={toggleMobileMenu}
                  className="text block py-2  text-lg capitalize font-light cursor-pointer menu-item"
                >
                  {item}
                </a>
              )
